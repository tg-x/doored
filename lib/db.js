...
DB.prototype.setDoorName = function (doorId, name, onSave)
{
    var door = this.getDoorByName(name)
    if (door)
    {
...
DB.prototype.setKeyAccess = function (keyIdOrName, doorIdOrName, level, onSave)
{
    ...
    this.save(onSave.bind({key: key, door: door}), {key: key, door: door});
    return true;
};
...
DB.prototype.setKeyName = function (keyIdOrName, name, onSave)
{
    ...
    this.conf.set(['keys', key.id, 'name'].join(':'), name)
    this.save(onSave);
    return true;
};
...
DB.prototype.save = function (onSave, args)
{
    if (this.saving)
    {
        this.saveAgain.push(onSave);
        return;
    }

    if (!(onSave instanceof Array))
    {
    ...
    this.conf.save(function(err) {
        ...
        {
            if (typeof onSave[i] == 'function')
                onSave[i](err, args);
        }
        if (db.saveAgain.length > 0)
        {
            db.save(db.saveAgain);
        }
