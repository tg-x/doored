...
var Server = function (port, host, log, db, masters, door, led)
{
    function resetLeds ()
    {
        led.red.write(OFF);
    ...
    var server = net.createServer(function (socket) {
            ...
            if (initDB) // initialize empty database: grant root access to the first key
            {
                ...
                for (var i = 0; i < masters.length; i++)
                {
                    for (var j = 0; j < masters[i].doors.length; j++)
                    {
                        ...
                        db.initDoor(d.id, d.id == door.id ? 'console' : d.id);
        ...
        function cmdHelp ()
        {
            ...
            socket.write('\nPrompts:\n'
                         ...
                         + '      Enter anything to abort the current command.\n'
                        );

            socket.write('\n');
        }

        function cmdDoorRename (doorIdOrName, name)
        {
            if (name.length > 8)
            {
            ...
            if (!db.setDoorName(doorIdOrName, name))
            {
            ...
            for (var i = 0; i < masters.length; i++)
            {
                for (var j = 0; j < masters[i].doors.length; j++)
                {
                    masters[i].doors[j].set({name: name});
                }
            }

            socket.write('Door '+ doorIdOrName +' renamed to '+ name +'.\n');
        }
        ...
        function printKeyHeader ()
        {
            ...
            for (var i = 0; i < masters.length; i++)
            {
                ...
                {
                    var d = masters[i].doors[j];
                    var name = db.getDoorName(d.id);
                    if (name != d.id) // only list named doors
                    {
                        doors[d.id] = name;
                        socket.write(printf(' % 8s', d.id));
                        door_names += printf(' % 8s', name);
                        n++;
                    }
                }
            }
            socket.write(printf('\n% -16s  % -10s % 5s %s\n', 'Key ID', 'Name', 'Door:', door_names));
        ...
        function printKeyEntry (key, doors)
        {
            ...
            for (var doorId in doors)
            {
                ...
                line += printf(' % 8s', level ? DB.XS_NAME[level] || '' : '');
            }
        ...
        function cmdKeyShow (keyIdOrName)
        {
            ...
            var doors = printKeyHeader();
            printKeyEntry(key, doors);
        }

        function cmdKeyList ()
        {
            var doors = printKeyHeader(doors);
            for (var id in db.getKeys())
                printKeyEntry(db.getKey(id), doors);
        }
